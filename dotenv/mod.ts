 ...
 *
 * console.log(os.env.get("GREETING"));
 * ```
 *
 * ```sh
 * > deno run --allow-env --allow-read app.ts
...
export interface ConfigOptions {
   ...
   * environment. Variables are then accessable via `os.env.get(<key>)`.
   *
   * @default {false}
   */
  export?: boolean;
...
// a guarded env access, that reads only a subset from the os.env object,
